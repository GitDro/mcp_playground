...
# Page config
st.set_page_config(
    page_title="MCP Playground (FastMCP Subprocess)",
    page_icon="⚡",
    layout="wide",
...
def create_function_schema_from_mcp_tools(mcp_tools: List[Dict]) -> List[Dict]:
    ...
    for tool in mcp_tools:
        ...
        if "search" in tool_name and "arxiv" not in tool_name:
...
with col2:
    # Clean header
    st.markdown("# **MCP Playground (FastMCP Subprocess)**")
    ...
    if models:
        ...
        with st.expander(f"{len(models)} models available, {tool_count} FastMCP tools available", expanded=False):
            ...
            if st.session_state.get('use_functions', True):
                st.markdown("**FastMCP Tools (Subprocess):**")
    ...
    if models:
        ...
        with col_func:
            st.session_state.use_functions = st.checkbox(
                ...
                help="Enable FastMCP tools via subprocess transport for web search, URL analysis, arXiv papers, finance, and YouTube"
            )
    ...
    if st.session_state.selected_model:
        ...
        # Clean empty state
        st.markdown("### Welcome to FastMCP Subprocess Version")
        ...
        with st.expander("What can I do?"):
            ...
            st.markdown("\n**Note:** This version uses FastMCP with PythonStdioTransport, running the server as a subprocess for true client-server separation.")
